...
public class WordAsset {
    ...
    public void paint(Graphics g) {
        ...
        g.drawString("Word: " + word, x, realY);
        lines++;

        for(int i = 0; i < phonetics.length; i++) {
            g.drawString(String.format("Phonetics (%d): %s", i+1, phonetics[i].getText()), x, realY + (height + LINE_SPACING) * lines++);
        }

        for(int i = 0; i < meanings.length; i++) {
            g.drawString(String.format("%d. Meaning", i+1), x, realY + (height + LINE_SPACING) * lines++);
            g.drawString(String.format("Part of Speech: %s", meanings[i].getPartOfSpeech()), x + 20, realY + (height + LINE_SPACING) * lines++);

            WordData.WordDefinition[] definitions = meanings[i].getDefinitions();
            for(int j = 0; j < definitions.length; j++) {
                g.drawString(String.format("%d. Definition", j+1), x + 20, realY + (height + LINE_SPACING) * lines++);

                List<String> wraps = wrap(definitions[j].getDefinition());
                g.drawString(String.format("Def.: %s", wraps.remove(0)), x + 40, realY + (height + LINE_SPACING) * lines++);
                for(String line : wraps) {
                    g.drawString(line, x + 40 + defPrefix, realY + (height + LINE_SPACING) * lines++);
                }

                if(definitions[j].getExample() != null) {
                    wraps = wrap(definitions[j].getExample());
                    g.drawString(String.format("Ex.: %s", wraps.remove(0)), x + 40, realY + (height + LINE_SPACING) * lines++);
                    for(String line : wraps) {
                        g.drawString(line, x + 40 + exPrefix, realY + (height + LINE_SPACING) * lines++);
                    }
                }

                String[] synonyms = definitions[j].getSynonyms();
                if(synonyms == null || synonyms.length == 0) continue;
                g.drawString("Synonyms", x + 40, realY + (height + LINE_SPACING) * lines++);
                for (String synonym : synonyms) {
                    g.drawString("- " + synonym, x + 60, realY + (height + LINE_SPACING) * lines++);
                }
            }
        }
    }
